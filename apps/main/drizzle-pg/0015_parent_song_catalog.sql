...
-- Parent attributes come from the preferred child row: latest gameVersion,
-- jp preferred over other regions — the same preference rule the song detail
-- queries use. The publicId is freshly generated: 8 chars over the nanoid
-- alphabet (48 bits; base64 of md5 with +/ mapped to -_), unique-guarded by
-- the parent_song_publicId_unique constraint.
