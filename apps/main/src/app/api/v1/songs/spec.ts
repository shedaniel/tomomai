...
export const spec = defineRoute({
  ...
  query: z.object({
    region: regionSchema,
    gameVersion: z.string().regex(/^-?(0|[1-9]\d*)$/)
      .pipe(z.coerce.number<string>().int().min(-32768).max(32767))
      .describe("Game version ID (e.g. 13; negative IDs are classic-era versions)."),
  }).refine(({ region, gameVersion }) => isCatalogVersion(region, gameVersion), {
    path: ["gameVersion"], message: "Unknown game version for this region",
  }),
  response: z.object({
