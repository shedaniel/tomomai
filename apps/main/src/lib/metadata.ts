...
function getReleaseTime(dateString: string, region: Region): number {
  // JP and international version rollovers remain at 7 AM JST, independent of maintenance.
  return parseDate(dateString).getTime() + (region === "cn" ? 0 : 7 * 60 * 60 * 1000);
}
...
/** Get the version current at an instant; JP/intl releases start at 7 AM JST. */
export function getVersionFromDate(date: Date, region: Region): VersionId {
  ...
  for (const version of sortedVersions) {
    const releaseTime = getReleaseTime(getRegionReleaseDate(version, region)!, region);
 ...
 *               Defaults to current time. JP/intl releases start at 7 AM JST.
...
export function isVersionAvailable(versionId: VersionId, region: Region, date: Date = new Date()): boolean {
  ...
  return date.getTime() >= getReleaseTime(dateString, region);
}

/**
 * Get a version by its short name.
 *
 * @param shortName - Short name of the version
 * @returns VersionInfo object or undefined if not found
 */
export function getVersionByShortName(shortName: string): VersionInfo | undefined {
  return Object.values(VERSIONS).find(version => version.shortName === shortName);
}

/**
 * Get a version by its short code.
 * For example 19995 will evaluate to "199", which is between FiNALE's 199 and DX's 200
 *
 * @param shortCode - Short code of the version
 * @returns VersionInfo object or undefined if not found
 */
export function getVersionByShortCode(shortCode: string): VersionInfo | undefined {
  ...
  // Find the version with the highest `version` value that is <= versionNum
