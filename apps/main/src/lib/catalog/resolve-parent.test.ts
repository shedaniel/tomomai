...
describe("resolveParents", () => {
  ...
  it("uses addedVersion to distinguish parents with the same artist", () => {
    const a = parent({ id: BigInt(1), childAddedVersions: new Set([1]) });
    const b = parent({ id: BigInt(2), disambiguator: 1, childAddedVersions: new Set([2]) });
    const result = resolveParents([song({ id: BigInt(3), region: "intl", addedVersion: 2 })], [a, b]);
    expect(result.assignments.get(BigInt(3))).toBe(b);
  });

  it("does not choose an arbitrary parent when matching evidence is ambiguous", () => {
    const a = parent({ id: BigInt(1) });
    const b = parent({ id: BigInt(2), disambiguator: 1 });
    const result = resolveParents([song({ id: BigInt(3), region: "intl" })], [a, b]);
    expect(result.newParents).toHaveLength(1);
    expect(result.newParents[0].disambiguator).toBe(2);
  });

  it("assigns stable disambiguators regardless of incoming order", () => {
    const batch = [song({ id: BigInt(1), artist: "B" }), song({ id: BigInt(2), artist: "A" })];
    const forward = resolveParents(batch, []);
    const reversed = resolveParents([...batch].reverse(), []);
    expect(forward.assignments.get(BigInt(1))?.disambiguator).toBe(reversed.assignments.get(BigInt(1))?.disambiguator);
    expect(forward.assignments.get(BigInt(2))?.disambiguator).toBe(reversed.assignments.get(BigInt(2))?.disambiguator);
  });

  it("reserves a known artist before a new colliding chart with an earlier addedVersion", () => {
    const existing = parent({ childAddedVersions: new Set([10]) });
    const result = resolveParents([
      song({ id: BigInt(10), gameVersion: 14, addedVersion: 10 }),
      song({ id: BigInt(11), gameVersion: 14, addedVersion: 1, artist: "New collision" }),
    ], [existing]);
    expect(result.assignments.get(BigInt(10))).toBe(existing);
    expect(result.assignments.get(BigInt(11))).toBe(result.newParents[0]);
    expect(result.newParents).toHaveLength(1);
    expect(result.newParents[0].artist).toBe("New collision");
  });

  it("reserves the exact addedVersion before a same-artist collision", () => {
    const existing = parent({ childAddedVersions: new Set([10]) });
    const result = resolveParents([
      song({ id: BigInt(10), gameVersion: 14, addedVersion: 10 }),
      song({ id: BigInt(11), gameVersion: 14, addedVersion: 1 }),
    ], [existing]);
    expect(result.assignments.get(BigInt(10))).toBe(existing);
    expect(result.assignments.get(BigInt(11))).toBe(result.newParents[0]);
  });
