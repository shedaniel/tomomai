...
export async function GET(request: NextRequest) {
  try {
    ...
    // Step 1: Query source chart instances based on criteria
    ...
    // (keyed by parentId — same chart, different region/version instance)
    const existingTargetParents = new Set<string>();

    if (mode === "only-upsert") {
      console.log("Step 2: Querying existing target songs for upsert mode...");
      const existingTargetSongs = await db
      ...
      for (const song of existingTargetSongs) {
        existingTargetParents.add(song.parentId.toString());
      }

      console.log(`Found ${existingTargetParents.size} existing charts in target`);
    }

    // Step 3: Prepare target rows. The parent stays the same — it's the same
    // chart — so no new parents and no publicId generation are needed.
    ...
    for (const sourceSong of sourceSongs) {
      if (mode === "only-upsert") {
        // Only include charts that already exist in target
        if (!existingTargetParents.has(sourceSong.parentId.toString())) {
          console.log(`Skipping new chart in upsert mode: parentId=${sourceSong.parentId}`);
          skippedCount++;
          continue;
        }
        updatedCount++;
      ...
      // Create new child row with target region/version; id is auto-generated
       
      const { id, ...songWithoutId } = sourceSong;
      targetSongs.push({
        ...songWithoutId,
