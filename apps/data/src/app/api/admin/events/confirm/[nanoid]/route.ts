...
export async function GET(
  ...
  const { log } = requestLogger(_request, "admin/events/confirm");
  try {
  ...
  return NextResponse.json({
    ...
    log.error({ err }, "Event confirmation failed");
    return NextResponse.json({ error: "Event confirmation failed" }, { status: 500 });
