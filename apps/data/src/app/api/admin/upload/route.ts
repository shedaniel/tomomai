import { restoreSongIds, retireSongs } from "@/server/services/admin/song-identities";
import { db } from "@/lib/db";
import { songs, parentSong } from "@/lib/db/schema";
...
import { findDuplicateUpload, matchUpload } from "@tomomai/catalog/match-upload";
...
async function applyChanges(
  ...
  const upsertBatch = async (rows: WriteRow[]) => {
    ...
    for (let i = 0; i < rows.length; i += batchSize) {
      const batch = await restoreSongIds(db, rows.slice(i, i + batchSize).map(row => pendingSongToChildValues(row, region, version)));
  ...
  for (let i = 0; i < deletions.length; i += 1000) {
    ...
    appliedDeleted += await retireSongs(db, ids);
  }
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const change of changes.deleted) change.playRecordCount = 0;
