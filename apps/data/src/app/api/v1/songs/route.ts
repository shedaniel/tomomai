...
export async function GET(request: NextRequest) {
  ...
  try { gameVersion = parseCatalogVersion(regionParam as Region, gameVersionParam ?? ""); }
  catch { return NextResponse.json({ error: "Invalid gameVersion" }, { status: 400 }); }

  const baseQuery = db
    .select({
      songId: parentSong.publicId,
      songName: parentSong.songName, artist: parentSong.artist, genre: parentSong.genre,
      cover: parentSong.cover, bpm: parentSong.bpm, type: parentSong.type,
      difficulty: parentSong.difficulty, disambiguator: parentSong.disambiguator,
      region: songs.region,
