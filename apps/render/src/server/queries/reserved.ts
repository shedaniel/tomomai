...
// Drop-in replacement for Next's `unstable_cache`: process-level TTL memo keyed
// by the call arguments. The render service is a long-lived process, so a plain
// in-memory cache gives us the same "recompute at most every revalidate seconds"
// behaviour without Next's data cache.
function ttlMemo<A extends unknown[], R>(
  fn: (...args: A) => Promise<R>,
  ttlMs: number,
): (...args: A) => Promise<R> {
  ...
  return (...args: A) => {
    ...
    // On rejection, drop the entry so the next call retries instead of caching the error.
...
const fetchReservedSongs = ttlMemo(
